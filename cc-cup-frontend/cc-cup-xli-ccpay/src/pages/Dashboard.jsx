...
const Dashboard = () => {
  ...
  if (loading) {
    return (<div className="bg-[#090a0b] text-[#f4f5f6] flex min-h-screen items-center justify-center font-sans"> <div className="flex flex-col items-center space-y-4"> <div className="w-8 h-8 border-4 border-[#69ff87] border-t-transparent rounded-full animate-spin"></div> <p className="text-xs font-bold text-[#8a939e] uppercase tracking-widest">Loading Ledger Details...</p> </div> </div>
    );
  }

  if (error || !dashboardData) {
    return (<div className="bg-[#090a0b] text-[#f4f5f6] flex min-h-screen items-center justify-center font-sans px-4"> <Card className="rounded-2xl border border-[#4a1d1d] bg-[#131619] p-6 max-w-sm w-full text-center"> <p className="text-sm font-bold text-[#ff6b6b] uppercase tracking-wide">Connection Error</p> <p className="text-xs text-[#8a939e] mt-2">{error || "Could not retrieve user context profiles."}</p>
      <Button
        onClick={() => window.location.reload()}
        className="mt-4 w-full bg-[#1a1d21] text-white border border-[#262b32] text-xs font-bold uppercase rounded-xl py-2"
      >
        Retry Connection </Button> </Card> </div>
    );
  }

  const { user, shift_start, shift_end, supervisor_name, transactions } = dashboardData;

  return (<div className="bg-[#090a0b] text-[#f4f5f6] flex min-h-screen font-sans antialiased selection:bg-[#69ff87]/30 w-full overflow-x-hidden"> <main className="flex-1 flex flex-col min-h-screen w-full relative">
    ...
    <header className="sticky top-0 z-40 bg-[#090a0b]/80 backdrop-blur-md border-b border-[#16191d] flex justify-between items-center w-full px-4 py-3.5 max-w-5xl mx-auto">
      ...
      <div className="flex items-center space-x-3">
        <button
          ...
          className="p-2 text-[#8a939e] hover:text-[#ff6b6b] bg-[#131619] rounded-full border border-[#1e2226] transition-all"
          aria-label="Logout"
          title="Logout"
        >
        ...
        <Avatar className="h-8 w-8 ring-2 ring-[#1e2226]">
          <AvatarImage
            src={`https://api.dicebear.com/7.x/initials/svg?seed=${user.first_name || 'ST'}`}
            alt="Student Profile"
          />
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 relative">
                ...
                <h2 className="text-[#8a939e] font-bold tracking-wider text-[11px] uppercase leading-none">
                  Balance
                </h2>
              </div>
            ...
            <div className="flex items-center space-x-2.5">
              <span className="text-3xl font-black tracking-tight text-white tabular-nums">
                {showBalance
                  ? `Rp ${user.current_saldo.toLocaleString('id-ID')}`
                  : "••••••"}
              </span>
        ...
        <section className="lg:col-span-5 bg-[#131619] border border-[#1e2226] p-5 rounded-2xl flex flex-col justify-between shadow-xl gap-4">
          ...
          <div className="flex items-center p-3 bg-[#090a0b] border border-[#1e2226] rounded-xl">
            ...
            <div className="min-w-0 flex flex-col space-y-0.5">
              <p className="text-[9px] uppercase tracking-wider text-[#535c66] font-bold leading-normal">
                Supervisor In Charge
              </p>
              <p className="text-xs font-bold text-white truncate leading-normal">
                {supervisor_name}
              </p>
            </div>
          </div>
        ...
        <Card className="rounded-2xl border border-[#1e2226] overflow-hidden p-0 bg-[#131619] shadow-xl">
          <div className="px-4 py-3.5 border-b border-[#1e2226] flex justify-between items-center bg-[#131619]">
            <div className="flex items-center space-x-2">
              ...
              <h3 className="text-xs font-bold tracking-wider text-[#f4f5f6] uppercase">
                Transaction History
              </h3>
            </div>
            ...
            {!transactions || transactions.length === 0 ? (
              ...
              transactions.map((txn, index) => {
                ...
                return (
                  <div
                    key={txn.id || index}
                    className="px-4 py-3 flex items-center justify-between hover:bg-[#171b20] transition-colors group"
                  >
                    <div className="flex items-center space-x-3 min-w-0">
                      <div
                        className={`h-8 w-8 rounded-lg flex items-center justify-center border flex-shrink-0 ${isIncoming
                    ...
                    <div className="text-right flex-shrink-0 ml-3">
                      <p
                        className={`font-bold text-xs tracking-tight ${isIncoming
                            ? 'text-[#69ff87]'
                            : 'text-[#ff6b6b]'
                          }`}
                      >
                      ...
                      <p className="font-mono text-[9px] text-[#535c66] uppercase mt-0.5 tracking-wider">
                        {txn.reference_id
                          ? txn.reference_id.substring(0, 8).toUpperCase()
                          : `TXN-${txn.id}`}
                      </p>
                    </div>
                  </div>
        ...
        <div className="bg-[#131619] border border-[#1e2226] p-4 rounded-2xl flex items-center justify-between shadow-md hover:bg-[#171b20] transition-all cursor-pointer group">
          <div className="flex items-center space-x-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[11px] text-[#8a939e] mt-0.5 truncate">
