...
const MerchantDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/ccpay/merchant/dashboard/?token=${encodeURIComponent(token)}`,
        {
          method: 'GET',
          headers: {
  ...
  const latestTxn =
    liveTransactions.length > 0 ? liveTransactions[0] : null;

  if (!token) {
    return (
      <div className="bg-[#090a0b] min-h-screen text-white flex items-center justify-center">
        <p className="text-sm text-[#8a939e]">
          Merchant token is missing.
        </p>
      </div>
    );
  }
    ...
    <div className="bg-[#090a0b] font-sans text-[#8a939e] min-h-screen pb-32 selection:bg-[#69ff87]/30 select-none antialiased w-full max-w-md mx-auto border-x border-[#16191d]/40 relative">
      ...
      <main className="px-4 pt-6 space-y-6">
        ...
        <section className="bg-gradient-to-b from-[#131619] to-[#0d0f11] border border-[#1e2226] rounded-2xl p-5 shadow-xl relative overflow-hidden">
          ...
          <p className="text-[9px] font-black text-[#535c66] uppercase tracking-widest mb-1">
        ...
        <section className="space-y-2.5">
          <p className="text-[9px] font-black text-[#535c66] uppercase tracking-widest px-0.5">
            DANA TERAKHIR MASUK
          </p>
            ...
            <div className="bg-gradient-to-b from-[#131619] to-[#131619]/90 rounded-2xl p-5 border border-[#1e2226] shadow-xl relative overflow-hidden">
              ...
              <div className="flex justify-between items-end">
                <div className="min-w-0">
                  <p className="text-[9px] font-bold text-[#535c66] uppercase tracking-wider mb-0.5">
                    Pelanggan/Siswa
                  </p>
        ...
        <section className="space-y-3">
          <div className="flex justify-between items-center px-0.5">
            <h2 className="text-[10px] font-black uppercase tracking-widest text-[#535c66]">
              Aliran Transaksi Live
            </h2>
          ...
          <div className="space-y-2">
            {isLoading ? (
              [1, 2, 3].map(i => (
                <Skeleton
                  key={i}
                  className="h-16 w-full rounded-xl bg-[#131619]"
                />
              ))
              ...
              liveTransactions.map((txn) => (
                <div
                  key={txn.id}
                  className="bg-[#131619]/40 border border-[#1e2226] p-3.5 rounded-xl flex justify-between items-center hover:border-[#2a2f35] transition-all cursor-pointer group"
                >
                  <div className="min-w-0 text-left">
                    <span className="text-xs font-bold text-white tracking-tight block truncate group-hover:text-[#69ff87] transition-colors">
                      {txn.name}
                    </span>

                    <span className="text-[10px] text-[#535c66] font-mono mt-0.5 block">
                      {txn.time} • ID: {txn.id}
                    </span>
                  </div>
              ...
              <div className="text-center p-4 text-xs text-[#535c66]">
        ...
        <section className="space-y-3">
          <h2 className="text-[10px] font-black uppercase tracking-widest text-[#535c66] px-0.5">
            Buku Riwayat Harian
          </h2>
          ...
          <div className="grid grid-cols-1 gap-2">
            {isLoading ? (
              ...
              dailyHistory.map((item, idx) => (
                <div
                  key={idx}
                  className="bg-[#131619]/20 border border-[#1e2226]/60 p-3.5 rounded-xl flex justify-between items-center"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      ...
                      <span className="text-[10px] text-[#535c66] block font-medium mt-0.5">
                        {item.count} Pembayaran
                      </span>
                    </div>
                  </div>
